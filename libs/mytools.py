...
def cv_rotate2(cv_img, angle):
    if angle != 0:       
        rotation_code = cv2.ROTATE_90_COUNTERCLOCKWISE if angle==90 else cv2.ROTATE_90_CLOCKWISE
        #Rotate the image to start processing
        myimage_rotated = cv2.rotate(cv_img, rotation_code, ) 
    else:
        myimage_rotated = cv_img
    return myimage_rotated


# Corrected my_read_image to load a 3-channel image robustly using PIL, then convert to OpenCV format
def my_read_image(path, scale=1.0):    
    # scale = 0.6 #0.6
